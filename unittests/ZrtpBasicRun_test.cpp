...
TEST_F(ZrtpBasicRunFixture, full_run_test_pq54) {
    // Configure with mandatory algorithms only
    auto aliceConfigure = make_shared<ZrtpConfigure>();
    auto bobConfigure = make_shared<ZrtpConfigure>();
    ...
    aliceConfigure->clear();
    bobConfigure->clear();
    aliceConfigure->addAlgo(PubKeyAlgorithm, zrtpPubKeys.getByName("PQ54"));
    bobConfigure->addAlgo(PubKeyAlgorithm, zrtpPubKeys.getByName("PQ54"));

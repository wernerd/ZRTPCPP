...
void sha256(const uint8_t* data, uint64_t const dataLength, uint8_t* digest) {
    auto const hash = Botan::HashFunction::create("SHA-256");
...
void sha256Ctx(void* ctx, const uint8_t* data, uint64_t const dataLength) {
    if (auto const* hd = static_cast<shaCtx *>(ctx); hd != nullptr) {

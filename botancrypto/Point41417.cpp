...
namespace Botan {
    std::vector<uint8_t> Point41417p::encode(Compression_Type const format) const {
        std::vector<uint8_t> result;

        EC41417_Group const m_group;

        const size_t p_bytes = m_group.get_p().bytes(); // Check: this should be 52

        auto [fst, snd] = getAffineXY();
        if (format == UNCOMPRESSED) {
            result.resize(1 + 2 * p_bytes);
            result[0] = 0x04;
            BigInt::encode_1363(&result[1], p_bytes, fst);
            BigInt::encode_1363(&result[1 + p_bytes], p_bytes, snd);
        }
        ...
        // append the X-coordinate
            ...
            result[0] = 0x02 | static_cast<uint8_t>(snd.get_bit(0));
            BigInt::encode_1363(&result[1], p_bytes, fst);
        } else {
    ...
    void Point41417p::swap(Point41417p &other) noexcept {
    ...
    BigInt Point41417p::decompress_point(bool const isOdd, const BigInt &xOrY) {
        EC41417_Group const m_group;
    ...
    bool Point41417p::on_the_curve() const {
        ...
        std::vector<BigInt> workspace(WORKSPACE_SIZE);
    ...
    Point41417p::getAffineXY() const {
        ...
        BigInt Rx = m_group.multiply_mod_prime(m_coord_x, z1);

        /* affine y = Y / Z */
        BigInt Ry = m_group.multiply_mod_prime(m_coord_y, z1);
    ...
    BOTAN_PUBLIC_API(2, 0) Point41417p operator*(const BigInt &scalar, const Point41417p &point) {
        ...
        Point41417p n(point);
        Point41417p R = Point41417p::zero();

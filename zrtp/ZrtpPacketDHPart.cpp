...
void ZrtpPacketDHPart::initialize() {
    ...
    zrtpHeader = &reinterpret_cast<DHPartPacket_t *>(data)->hdr; // the standard header
...
void ZrtpPacketDHPart::setPacketLength(size_t const pubKeyLen) {
    dhLength = pubKeyLen;
    ...
    auto const length = static_cast<uint16_t>(sizeof(DHPartPacket_t) + roundUp +
                                        2 * ZRTP_WORD_SIZE); // HMAC field is 2*ZRTP_WORD_SIZE
    setLength(static_cast<uint16_t>(length / ZRTP_WORD_SIZE));
}

ZrtpPacketDHPart::ZrtpPacketDHPart(uint8_t const* data, bool const isNpAlgorithm) {
    zrtpHeader = const_cast<zrtpPacketHeader_t *>(&reinterpret_cast<DHPartPacket_t const *>(data)->hdr); // the standard header
    DHPartHeader = const_cast<DHPart_t*>(&reinterpret_cast<DHPartPacket_t const *>(data)->dhPart);

    pv = const_cast<uint8_t *>(data + sizeof(DHPartPacket_t)); // point to the public key value(s)

    auto const isDhPart2 = getMessageTypeString() == DHPart2Msg;

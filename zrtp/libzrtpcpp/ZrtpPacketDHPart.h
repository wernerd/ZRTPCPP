...
#ifndef ZRTPPACKETDHPART_H_
#define ZRTPPACKETDHPART_H_
...
class __EXPORT ZrtpPacketDHPart final : public ZrtpPacketBase {
public:
    ...
    /// In case of new NP algorithms then it has to be 21 or greater.
    [[nodiscard]] bool isLengthOk(bool const isNpAlgorithmActive) const {
        return !isNpAlgorithmActive ? getLength() >= 29 : getLength() >= 21;
    }
    ...
    void setRs1Id(zrtp::RetainedSecArray const &text) {
        memcpy(DHPartHeader->rs1Id, text.data(), sizeof(DHPartHeader->rs1Id));
    }

    // ReSharper disable once CppMemberFunctionMayBeConst
    /// Set second retained secret id, fixed length byte array
    void setRs2Id(zrtp::RetainedSecArray const &text) {
        memcpy(DHPartHeader->rs2Id, text.data(), sizeof(DHPartHeader->rs2Id));
    }

    // ReSharper disable once CppMemberFunctionMayBeConst
    /// Set additional retained secret id, fixed length byte array
    void setAuxSecretId(zrtp::RetainedSecArray const &text) {
        memcpy(DHPartHeader->auxSecretId, text.data(), sizeof(DHPartHeader->auxSecretId));
    }

    // ReSharper disable once CppMemberFunctionMayBeConst
    /// Set PBX retained secret id, fixed length byte array
    void setPbxSecretId(zrtp::RetainedSecArray const &text) {
        memcpy(DHPartHeader->pbxSecretId, text.data(), sizeof(DHPartHeader->pbxSecretId));
    }
...
private:
    ...
    DHPart_t* DHPartHeader = &reinterpret_cast<DHPartPacket_t *>(data)->dhPart; ///< points to DH message structure
    uint8_t* pv = static_cast<uint8_t *>(data) + sizeof(DHPartPacket_t); ///< points to public key value inside DH message

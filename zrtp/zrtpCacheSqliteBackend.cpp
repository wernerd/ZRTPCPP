...
#include <sqlite3.h>        // Note: set the correct include path for compiler
...
constexpr int32_t localZidStandard = 1; /* this local ZID is not tied to a specific account */
constexpr int32_t localZidWithAccount = 2;
...
constexpr char createZrtpIdOwn[] =
        "CREATE TABLE zrtpIdOwn(localZid CHAR(18), type INTEGER, accountInfo VARCHAR(1000));";
 ...
 * of local, remote ZIDs and optional account information.
 */
...
static int b64Encode(const uint8_t* binData, size_t binLength, char* b64Data, size_t const b64Length) {
    (void)b64Length;
    size_t consumed = 0;
...
/**
 * Create ZRTP cache tables in the database.
 *
...
static int insertRemoteZidRecord(void* vdb, const uint8_t* remoteZid, const uint8_t* localZid,
    ...
    auto* db = static_cast<sqlite3 *>(vdb);
    sqlite3_stmt* stmt;
    int rc;

    char b64RemoteZid[IDENTIFIER_LEN * 2] = {};
    char b64LocalZid[IDENTIFIER_LEN * 2] = {};
...
static int readRemoteZidRecord(void* vdb, const uint8_t* remoteZid, const uint8_t* localZid,
    ...
    /* Getting data from the result set: column index starts with 0 (zero), not one */
...
static int readLocalZid(void* vdb, uint8_t* localZid, const char* accountInfo, char* errString) {
    ...
    /* Loop over the result set and count it. However, use only the localZid of the first row */
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (found == 0) {
            auto const zidBase64Text = reinterpret_cast<char const *>(sqlite3_column_text(stmt, 0));
    ...
    if (found == 0) {
        char b64zid[IDENTIFIER_LEN + IDENTIFIER_LEN] = {};

        /* create a 12-byte random value, convert to base 64, insert in zrtpIdOwn table */
        randomZRTP(localZid, IDENTIFIER_LEN);
        int const b64len = b64Encode(localZid, IDENTIFIER_LEN, b64zid, IDENTIFIER_LEN + IDENTIFIER_LEN);
...
static int openCache(const char* name, void** vpdb, char* errString) {
    sqlite3_stmt* stmt;
    int found = 0;
    auto** pdb = reinterpret_cast<sqlite3 **>(vpdb);
...
static int clearCache(void* vdb, char* errString) {
    ...
    if (int const rc = createTables(db, errString))
        return rc;
    return SQLITE_OK;
}
...
static int insertZidNameRecord(void* vdb, const uint8_t* remoteZid, const uint8_t* localZid,
    ...
    SQLITE_CHK(sqlite3_bind_int64(stmt, 5, time(nullptr)))
...
static int readZidNameRecord(void* vdb, const uint8_t* remoteZid, const uint8_t* localZid,
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        strncpy(zidName->name, reinterpret_cast<char const *>(sqlite3_column_text(stmt, 2)), zidName->nameLength);
    ...
    if (found == 0)
        ...
        if (errString)
            snprintf(errString, DB_CACHE_ERR_BUFF_SIZE,
                     "ZRTP name cache inconsistent. More than one ZID name found: %d\n", found);
        return 1;
    }
    return SQLITE_OK;
...
static void* prepareReadAllZid(void* vdb, char* errString) {
    ...
    SQLITE_CHK(
        SQLITE_PREPARE(db, selectZrtpIdRemoteAllNoCondition, sizeof(selectZrtpIdRemoteAllNoCondition), &stmt, nullptr))
    return stmt;
...
static void* readNextZidRecord(void* vdb, void* vstmt, remoteZidRecord_t* remZid, char* errString) {
    ...
    if (vstmt == nullptr)
        return nullptr;
    auto const stmt = static_cast<sqlite3_stmt *>(vstmt);
    ...
    if ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        remZid->secureSince = sqlite3_column_int64(stmt, 9);
        remZid->preshCounter = sqlite3_column_int(stmt, 10);
        auto const zidBase64Text = reinterpret_cast<char const *>(sqlite3_column_text(stmt, 11));
...
static void closeStatement(void* const vstmt) {
    if (vstmt == nullptr)
        return;
    auto const stmt = static_cast<sqlite3_stmt *>(vstmt);
    sqlite3_finalize(stmt);

...
TEST_F(SrtpCryptoTestFixture, f8_test) {
    auto const aesCipher = make_unique<SrtpSymCrypto>(SrtpEncryptionAESF8);
    auto const f8AesCipher = make_unique<SrtpSymCrypto>(SrtpEncryptionAESF8);
    ...
    auto* ui32p = reinterpret_cast<uint32_t *>(derivedIv);

...
typedef struct zrtp_Callbacks {
    ...
    */
    int32_t (*zrtp_sendDataZRTP)(ZrtpContext const* ctx, const uint8_t* data, int32_t length);
    ...
    */
    int32_t (*zrtp_sendFrameDataZRTP)(ZrtpContext const* ctx, const uint8_t* data, int32_t length,
    ...
    */
    int32_t (*zrtp_activateTimer)(ZrtpContext const* ctx, int32_t time);
    ...
    */
    int32_t (*zrtp_cancelTimer)(ZrtpContext const* ctx);
    ...
    */
    void (*zrtp_sendInfo)(ZrtpContext const* ctx, int32_t severity, int32_t subCode);
     ...
     */
    int32_t (*zrtp_srtpSecretsReady)(ZrtpContext const* ctx, C_SrtpSecret_t* secrets, int32_t part);
     ...
     *    switch on security
     */
    void (*zrtp_srtpSecretsOff)(ZrtpContext const* ctx, int32_t part);
     ...
     */
    void (*zrtp_rtpSecretsOn)(ZrtpContext const* ctx, char* c, char* s, int32_t verified);
     ...
     *    Pointer to the opaque ZrtpContext structure.
     */
    void (*zrtp_handleGoClear)(ZrtpContext const* ctx);
     ...
     * @see ZrtpCodes#MessageSeverity
     */
    void (*zrtp_zrtpNegotiationFailed)(ZrtpContext const* ctx, int32_t severity, int32_t subCode);
     ...
     *
     */
    void (*zrtp_zrtpNotSuppOther)(ZrtpContext const* ctx);
     ...
     *    Pointer to the opaque ZrtpContext structure.
     */
    void (*zrtp_synchEnter)(ZrtpContext const* ctx);
     ...
     *    Pointer to the opaque ZrtpContext structure.
     */
    void (*zrtp_synchLeave)(ZrtpContext const* ctx);
     ...
     */
    void (*zrtp_zrtpAskEnrollment)(ZrtpContext const* ctx, int32_t info);
     ...
     *    of an enrollment.
     */
    void (*zrtp_zrtpInformEnrollment)(ZrtpContext const* ctx, int32_t info);
     ...
     *
     */
    void (*zrtp_signSAS)(ZrtpContext const* ctx, uint8_t* sas);
     ...
     *
     */
    int32_t (*zrtp_checkSASSignature)(ZrtpContext const* ctx, uint8_t* sas);
} zrtp_Callbacks;
 ...
 *
 * @see zrtp_setStandardConfig(ZrtpContext const *  zrtpContext)
 * @see zrtp_setMandatoryOnly(ZrtpContext const *  zrtpContext)
 * @see zrtp_confClear(ZrtpContext const *  zrtpContext)
 * @see zrtp_containsAlgo(ZrtpContext const *  zrtpContext, Zrtp_AlgoTypes algoType, const char*  algo)
 * @see zrtp_setTrustedMitM(ZrtpContext const *  zrtpContext, int32_t yesNo);
 * @see zrtp_isTrustedMitM(ZrtpContext const *  zrtpContext);
 * @see zrtp_setSasSignature(ZrtpContext const *  zrtpContext, int32_t yesNo);
 * @see zrtp_isSasSignature(ZrtpContext const *  zrtpContext);
 */
 ...
 */
int32_t zrtp_CheckCksum(uint8_t const* buffer, uint16_t length, uint32_t crc);
 ...
 *    The computed CRC.
 */
uint32_t zrtp_GenerateCksum(uint8_t const* buffer, uint16_t length);
 ...
 */
void zrtp_processZrtpMessage(ZrtpContext const* zrtpContext, uint8_t const* extHeader, uint32_t peerSSRC,
                             size_t length);
 ...
 */
void zrtp_setAuxSecret(ZrtpContext const* zrtpContext, uint8_t const* data, int32_t length);
 ...
 */
int32_t zrtp_inState(ZrtpContext const* zrtpContext, int32_t state);
 ...
 */
char* zrtp_getHelloHash(ZrtpContext const* zrtpContext, int32_t index);
 ...
 */
void zrtp_setMultiStrParams(ZrtpContext const* zrtpContext, char const* parameters, int32_t length,
 ...
 */
void zrtp_acceptEnrollment(ZrtpContext const* zrtpContext, int32_t accepted);
 ...
 */
void zrtp_setEnrollmentMode(ZrtpContext const* zrtpContext, int32_t enrollmentMode);
 ...
 */
int32_t zrtp_setSignatureData(ZrtpContext const* zrtpContext, uint8_t const* data, int32_t length);
 ...
 */
const uint8_t* zrtp_getSignatureData(ZrtpContext const* zrtpContext);
 ...
 */
char** zrtp_getAlgorithmNames(ZrtpContext const* zrtpContext, Zrtp_AlgoTypes type);
 ...
 *    Number of free configuration data slots or -1 on error
 */
int32_t zrtp_addAlgoAt(ZrtpContext const* zrtpContext, Zrtp_AlgoTypes algoType, const char* algo, int32_t index);
 ...
 *
 */
const char* zrtp_getAlgoAt(ZrtpContext const* zrtpContext, Zrtp_AlgoTypes algoType, int32_t index);
 ...
 */
void zrtp_setTrustedMitM(ZrtpContext const* zrtpContext, int32_t yesNo);
 ...
 */
void zrtp_setSasSignature(ZrtpContext const* zrtpContext, int32_t yesNo);

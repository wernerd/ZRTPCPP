...
/**
 * @brief Test SIDH 751 functions inside ZRTP lib.
 *
 * These are the same tests as in SIDH unit test directory, only using the
 * ZRTP environment now.
 */
class ZrtpSidh751: public ::testing::Test {
public:
    ZrtpSidh751() = default;

    ZrtpSidh751(const ZrtpSidh751& other) = delete;
    ZrtpSidh751(const ZrtpSidh751&& other) = delete;
    ZrtpSidh751& operator= (const ZrtpSidh751& other) = delete;
    ZrtpSidh751& operator= (const ZrtpSidh751&& other) = delete;
    ...
    ~ZrtpSidh751( ) override {
        // cleanup any pending stuff, but no exceptions allowed
    }
};
#ifndef SIDH_COMPRESSED_WDI
TEST_F(ZrtpSidh751, FieldLengths) {
...
TEST_F(ZrtpSidh751, EphemeralSecret) {
    auto lengths = SidhWrapper::getFieldLengths(SidhWrapper::P751);
...
#else
TEST_F(ZrtpSidh751, FieldLengths) {
    auto lengths = SidhWrapper::getFieldLengths(SidhWrapper::P751Comp);
    ASSERT_EQ(SIDH_SECRETKEYBYTES_A, lengths->privateKeyA);
    ASSERT_EQ(SIDH_SECRETKEYBYTES_B, lengths->privateKeyB);
    ASSERT_EQ(SIDH_PUBLICKEYBYTES, lengths->publicKey);
    ASSERT_EQ(SIDH_BYTES, lengths->sharedSecret);
...
TEST_F(ZrtpSidh751, EphemeralSecret) {
    ...
    SidhWrapper::random_mod_order_A(SidhWrapper::P751Comp, privateKeyA.get());
    SidhWrapper::random_mod_order_B(SidhWrapper::P751Comp, privateKeyB.get());

    SidhWrapper::EphemeralKeyGeneration_A(SidhWrapper::P751Comp, privateKeyA.get(), publicKeyA.get());
    SidhWrapper::EphemeralKeyGeneration_B(SidhWrapper::P751Comp, privateKeyB.get(), publicKeyB.get());

    // Alice computes her shared secret using Bob's public key
    SidhWrapper::EphemeralSecretAgreement_A(SidhWrapper::P751Comp, privateKeyA.get(), publicKeyB.get(), sharedA.get());

    // Bob computes his shared secret using Alice's public key
    SidhWrapper::EphemeralSecretAgreement_B(SidhWrapper::P751Comp, privateKeyB.get(), publicKeyA.get(), sharedB.get());
